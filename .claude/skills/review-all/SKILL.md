---
name: review-all
description: Run all specialized reviewers on recent changes and aggregate results.
...
3. Run the **relevant** reviewers in parallel based on changed files:
   - Backend `.py` files changed -> **arch-guardian**, **security-auditor**
   - Frontend `.ts`/`.tsx` files changed -> **ui-guardian**
   - Test files changed -> **test-inspector**
   - Mixed -> all applicable reviewers
4. Aggregate all findings into a single report
5. Signal review completion:
   ```bash
...
- Only run reviewers relevant to the changed files
