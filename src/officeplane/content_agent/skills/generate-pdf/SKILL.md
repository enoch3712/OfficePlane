...
1. Loads the requested source documents from Postgres via Prisma (chapters,
   sections, summaries).
2. Prompts DeepSeek v4-flash to produce an **agnostic Document JSON tree**
...
5. Converts the `.docx` to `.pdf` using `libreoffice --headless` (already
   installed in the api Docker image — no extra pip deps required).
6. Writes the result to `/data/workspaces/<job_id>/output.pdf`.
