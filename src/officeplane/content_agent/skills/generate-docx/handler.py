"""Handler for the generate-docx skill.

Generates a Microsoft Word document from ingested source documents using
the agnostic Document tree (CommonMark/Pandoc-aligned).
...
PROMPT_TEMPLATE = """You are writing a Microsoft Word document for OfficePlane.
...
# ---------------------------------------------------------------------------
# Source loading helpers (copied from generate-docx-blocks so legacy can be
# deleted independently in Task 11)
# ---------------------------------------------------------------------------
...
async def execute(*, inputs: dict[str, Any], progress=None, **_) -> dict[str, Any]:
    ...
    try:
        ...
        model = model_for_skill("generate-docx")
